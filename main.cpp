...
void inorderTara(struct node *root);
void preorderTara(struct node *root);
void postorderTara(struct node *root);
...
void Search() {
  ...
  switch (aramaTur) {
  case 'I':
    ilkGenislikAra(temp, atoi(studenNo));
    break;
  case 'G':
...
void preorderTara(struct node *root) {
  if (root != NULL) {
    printNode(root);
    preorderTara(root->left);
    preorderTara(root->right);
  }
}
...
void postorderTara(struct node *root) {
  if (root != NULL) {
    postorderTara(root->left);
    postorderTara(root->right);
    printNode(root);
  }
}
...
void ilkGenislikAra(struct node *root, int studenNo) {
  if (root != NULL) {
    ilkGenislikAra(root->right, studenNo);
    if (root->studenNo == studenNo) {
      printNode(root);
      arama = 1;
    }
    ilkGenislikAra(root->left, studenNo);
  }
}
